...
@section('advantages')
    <div class="profile-image">
        <form @if (isset($advantages->id)) action="{{route('advantages_edit')}}" @else action="{{route('advantages_create')}}" @endif method="POST" enctype="multipart/form-data" id="save">
            @csrf
            @if($advantages->id)
                @method('POST')
            @endif
                                ...
                                <div class="row">
                                    <div class="container">
                                        <div class="form-group">
                                            ...
                                            <input id="text" type="text" name="text" value="{{$advantages['text']}}"
                                    ...
                                    <div class="container">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control mb-3"
                                                   ...
                                                   value="{{$advantages['sub_text']}}">
                                        </div>
                                    </div>
                                    <input type="hidden" name="id" value="{{$advantages['id']}}">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        @if(isset($advantages->id))
            <div class="" style="float: right;height: auto">
                <form method="POST" action="{{route('delete_advantages', ['id' => $advantages['id']])}}">
                    @csrf
                    {{method_field('DELETE')}}
