...
@section('skills')
    <div class="profile-image">
        <form @if (isset($skills->id)) action="{{route('skills_edit')}}" @else action="{{route('skills_create')}}" @endif method="POST" enctype="multipart/form-data" id="save">
            @csrf
            @if($skills->id)
                @method('POST')
            @endif
            <div class="col-md-12">
                <div class="mb-3">
                    <div class="col-lg-10 col-sm-12 mt-3 pl-0">
                        <input id="itn-croppie-result-i" type="text" hidden value="{{$skills['image']}}"
                               name="image">
                        <img id="itn-croppie-result" src="{{asset('/storage/'.$skills['image'])}}"
                                ...
                                <div class="row">
                                    <div class="container">
                                        <div class="form-group">
                                            ...
                                            <input id="text" type="text" name="text" value="{{$skills['text']}}"
        ...
        @if(isset($skills->id))
            <div class="" style="float: right;height: auto">
                <form method="POST" action="{{route('delete_skills', ['id' => $skills['id']])}}">
                    @csrf
                    {{method_field('DELETE')}}
